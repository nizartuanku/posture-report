...
//   - Posture Report only reads the other tools' databases; explaining a
//     finding here never writes anything back to them.
...
import (
	...
	"github.com/nizartuanku/posture-report/internal/aiclient"
)
...
func (s *Server) registerAI(mux *http.ServeMux) {
	...
	mux.HandleFunc("GET /api/priorities", s.handlePriorities)
}

// priorityJSON is one "fix first" item on the dashboard. Posture Report reads
// findings from the other Hexward tools' databases; it never changes them.
...
func (s *Server) handlePriorities(w http.ResponseWriter, r *http.Request) {
	rep := s.Load()
	out := make([]priorityJSON, 0, len(rep.TopPriorities))
	for _, f := range rep.TopPriorities {
		out = append(out, priorityJSON{f.Fingerprint, f.Module, f.Target, f.Check, f.Title, string(f.Severity), f.Remediation})
	}
	writeJSON(w, out)
}

// findFinding looks a finding up by fingerprint in a freshly loaded report
// (open findings and manual-review notes).
func (s *Server) findFinding(fingerprint string) (core.Finding, bool) {
	rep := s.Load()
	for _, list := range [][]core.Finding{rep.AllFindings, rep.ManualReviews} {
		for _, f := range list {
...
func (s *Server) aiAllowed() (bool, string) {
	...
	if t := strings.ToLower(s.Tier); s.AI.Keyed && t != "pro" && t != "team" {
...
func (s *Server) handleExplainFinding(w http.ResponseWriter, r *http.Request) {
	...
	rec, found := s.findFinding(req.Fingerprint)
	if !found {
	...
	packet, err := aiclient.NewFindingPacket("posture-report", s.AI.Language, aiclient.CoreFinding{
